...
export async function fetchPayoutRequests(params = {}) {
  ...
  return {
    requests: (payload.requests || []).map((r) => ({
      ...
      autoGenerated: Boolean(r.autoGenerated),
...
// generated automatically on `cycle` and `nextRunAt`, and the manual
// withdrawal windows no longer apply.

export async function fetchPayoutSettings() {
  const response = await api.get("/finance/payout-settings", { skipGlobalErrorHandler: true });
  return response.data?.data || {};
}

export async function updatePayoutSettings(cycle) {
  const response = await api.patch("/finance/payout-settings", { cycle }, { skipGlobalErrorHandler: true });
  return response.data?.data || {};
}
